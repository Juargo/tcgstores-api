...
export function getPrice(price: string, type: string): number {
  if (type === "normal") {
    // console.log("price", price);
    const numeros: number[] = [];
    price = price.replace(/\$/gi, " $");
    // console.log("precio agregando espacio entre $", `'${price}'`);
    const precios: string[] = price
      .split(" ")
      .map((x) => x.replace(/\n/gi, ""))
      .filter((x) => x != "");
...
export function Filters(Product: DataProduct): unknown {
  const filter = [
  ...
  return (
    !filter.includes(Product.nombre) &&
    Product.nombre !== "" &&
